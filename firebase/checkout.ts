...
import type { CompleteCheckoutParams, StockRead } from "@/types";
...
export async function completeCheckout({
  ...
  await runTransaction(db, async (tx) => {
    ...
    const paymentEnabled =
      userData.nextillApp?.settings?.paymentMethodSelectionEnabled === true;

    const currentTicketNumber = Number(dayCycle.nextTicketNumber ?? 1);
    ...
    const currentGlobalItemsSales = currentGlobalStats.itemsSales ?? {};
    ...
    // ── Prepare optional payment earnings update
    const paymentEarningsUpdate: Record<string, number> = {};

    if (paymentEnabled && paymentMethod) {
      const field =
        paymentMethod === "card" ? "cardEarnings" : "cashEarnings";

      const current = summarySnap.exists()
        ? Number(summarySnap.data()[field] ?? 0)
        : 0;

      paymentEarningsUpdate[field] = current + totalMinor;
    }

    // ── Daily summary (total earnings ALWAYS updated)
    tx.set(
      daySummaryRef,
      {
        date: dayKey,
        earnings: currentDayEarnings + totalMinor, // ✅ unchanged behavior
        ...
        updatedAt: serverTimestamp(),
        ...paymentEarningsUpdate, // ✅ optional
      },
      { merge: true }
    );
    ...
    tx.update(userRef, {
      "nextillApp.dayCycle.nextTicketNumber": nextTicketNumber,
