    ...
    >
      {topItems.map(([id, qty], i) => {
        ...
        return (
          <div
            key={id}
            className={`flex min-h-[140px] flex-col rounded-2xl border bg-surface-1 p-4 shadow-sm ${styles.card}`}
          >
            <div className="flex justify-center">
              <div
                className={`flex h-10 w-10 items-center justify-center rounded-full border text-sm font-semibold ${styles.badge}`}
              >
                {rank}
              </div>
            </div>
